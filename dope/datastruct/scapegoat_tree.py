...
class SGNode:
    '''
    A binary scapegoat tree node with data  
    '''
...
class SGTree:
    '''
    A scapegoat tree.  Alpha is the rebalancing constant which 
    determines how far from balanced the tree can approach before 
    causing a rebalance.  Global size must be maintained
    '''
...
def search(node,val):
    '''
    Search a scapegoat node with comparable data values for val
    '''
    if node is None:
        return False
    elif node.data == val:
...
def is_scapegoat(sgtree,node):
    '''
    Determine if this node has an imbalance between left and right
    children as determined by scapegoat tree rules
    '''
...
def _build_list(node):
    ''' 
    Build a sorted list of data from scapegoat tree node
    '''
    if node is None:
        return []
    else:
...
def _build_tree(sorted_list):
    '''
    Provided a sorted list create a balanced scapegoat tree and return
    the root
    '''
...
def rebalance(node):
    '''
    Rebalance the tree rooted at the provided node and return the root
    '''
